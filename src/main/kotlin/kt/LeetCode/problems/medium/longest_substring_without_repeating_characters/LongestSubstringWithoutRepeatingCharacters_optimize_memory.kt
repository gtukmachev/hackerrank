...
class Solution {
    ...
    fun lengthOfLongestSubstring(s: String): Int {
        ...
        for (i in 0 until L) {
            ...
            start = java.lang.Math.max(0, i - CHARACTERS_NUMBER + 1)
            ...
            for (p in start..i) {
                ...
                if (ns >= 0) { // skip excluded
                    ns = ns xor chMask
                    when{
                        (ns and chMask) == 0 -> { maxLength = java.lang.Math.max(maxLength, bitsCount(ns) + 1); sets[setsIndex] = -1 }  //excluding
            ...
            if ( sets[setsPos] >= 0 ) {
                maxLength = java.lang.Math.max(maxLength, bitsCount(sets[setsPos]))
                sets[setsPos] = 0;
            }
        ...
        for (p in 0 until CHARACTERS_NUMBER) {
            val n = sets[p];
            if (n >= 0)
                maxLength = java.lang.Math.max(maxLength, bitsCount(sets[p]))
        }
