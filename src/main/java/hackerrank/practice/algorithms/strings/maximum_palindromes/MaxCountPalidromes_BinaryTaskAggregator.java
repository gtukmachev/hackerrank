 ...
 */
public class MaxCountPalidromes_BinaryTaskAggregator {
    ...
    private static void initialize(String s) {
        MaxCountPalidromes_BinaryTaskAggregator.s = s.toCharArray();
        lettersFrequencyAggregator = new BinaryTaskAggregator<Map<Character, Integer>>(
                (byte)4,
                (iFrom, iTo) -> {
                    Map<Character, Integer> ch = new HashMap<>(1);
                    for (int i = iFrom; i <= iTo; i++) {
                        ch.merge(MaxCountPalidromes_BinaryTaskAggregator.s[i], 1, (a, b) -> a + b);
                    }
                    return ch;
...
//                    System.out.println();
        ...
        private Path getFileForTest(String sufix) {
            return Paths.get("src/main/java/" + MaxCountPalidromes_BinaryTaskAggregator.class.getName().replace(".", "/") + "-" + sufix);
        }
